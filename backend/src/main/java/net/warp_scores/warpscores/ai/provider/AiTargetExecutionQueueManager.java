...
public class AiTargetExecutionQueueManager {
    ...
    private final class TargetQueue {
        ...
        QueuedCall take() throws InterruptedException {synchronized(monitor){while(!stopping){Instant now=Instant.now();Instant blocked=quota(group()).blockedUntil();QueuedCall best=pending.stream().filter(c->!c.result.isDone()).filter(c->c.availableAt==null||!c.availableAt.isAfter(now)).filter(c->blocked==null||!blocked.isAfter(now)).max(Comparator.comparingInt((QueuedCall c)->c.priority).thenComparing(c->c.createdAt,Comparator.reverseOrder())).orElse(null);if(best!=null){pending.remove(best);best.status=JobStatus.RUNNING;activeJobs.put(best.id,best);return best;}Instant wake=blocked;for(QueuedCall c:pending)if(c.availableAt!=null&&(wake==null||c.availableAt.isBefore(wake)))wake=c.availableAt;if(wake==null)monitor.wait();else monitor.wait(Math.max(1L,Duration.between(now,wake).toMillis()));}return null;}}
        void run(QueuedCall c){running.incrementAndGet();boolean admitted=false;long start=System.nanoTime();try{try{admission.acquire(c.agentId,c.request);admitted=true;}catch(AiGenerationAdmissionService.AdmissionDeniedException d){if(d.reason()==AiGenerationAdmissionService.DenialReason.CONCURRENCY_LIMIT){c.status=JobStatus.QUEUED;c.availableAt=Instant.now().plusMillis(500);enqueue(c);return;}throw d;}LlmProvider p=registry.require(c.target.providerId());if(!p.isConfigured())throw new IllegalStateException("AI provider is not configured: "+c.target.providerId());c.attempts++;CanonicalLlmResponse response=p.generate(c.request);long duration=elapsed(start);traceStore.recordSuccess(c.agentId,c.target.providerId(),c.request,response,duration);c.status=JobStatus.SUCCEEDED;recordCompletion(duration);succeeded.incrementAndGet();c.result.complete(response);}catch(LlmProviderException e){long duration=elapsed(start);traceStore.recordFailure(c.agentId,c.target.providerId(),c.request,e,duration);remember(e);if(e.kind()==LlmProviderException.Kind.RATE_LIMIT&&c.attempts<Math.max(1,config.getQueue().getMaxAttempts())){Instant retry=e.retryAt();if(retry==null||!retry.isAfter(Instant.now()))retry=Instant.now().plus(backoff(c.attempts));quota(group()).blockUntil(retry,e.getMessage());c.status=JobStatus.RETRY_WAIT;c.availableAt=retry;c.lastError=shortError(e);enqueue(c);return;}c.status=JobStatus.FAILED;c.lastError=shortError(e);recordCompletion(duration);failed.incrementAndGet();c.result.completeExceptionally(e);}catch(RuntimeException e){long duration=elapsed(start);traceStore.recordUnexpectedFailure(c.agentId,c.target.providerId(),c.request,e,duration);lastError=shortError(e);lastErrorAt=Instant.now();c.status=JobStatus.FAILED;c.lastError=shortError(e);recordCompletion(duration);failed.incrementAndGet();c.result.completeExceptionally(e);}finally{if(admitted)admission.release();activeJobs.remove(c.id);running.decrementAndGet();}}
        ...
        QueueSnapshot snapshot(){
            ...
            synchronized(monitor){jobs=java.util.stream.Stream.concat(pending.stream(),activeJobs.values().stream()).distinct().sorted(Comparator.comparingInt((QueuedCall c)->c.priority).reversed().thenComparing(c->c.createdAt)).map(QueuedCall::snapshot).toList();}
            ...
            if(resume!=null&&blocked!=null&&blocked.isAfter(resume))resume=blocked;
        ...
        void recordCompletion(long durationMs){completedMillis.addAndGet(Math.max(0,durationMs));completed.incrementAndGet();completedHistory.addLast(Instant.now());pruneHistory(Instant.now());}
        void pruneHistory(Instant now){Instant cutoff=now.minus(Duration.ofHours(24));while(true){Instant first=completedHistory.peekFirst();if(first==null||!first.isBefore(cutoff))return;completedHistory.pollFirst();}}
    ...
    public record QueueSnapshot(String target,String provider,String model,String quotaGroup,int concurrency,int queued,int running,long retryWaiting,long succeeded,long failed,Instant blockedUntil,String throttleReason,String lastError,Integer lastStatusCode,Instant lastErrorAt,Instant historyStartedAt,long completedLastHour,long completedLast24Hours,Long averageDurationMs,Instant resumeAt,Long estimatedClearSeconds,List<JobSnapshot> jobs){}
