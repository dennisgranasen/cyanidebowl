...
public class AiTargetExecutionQueueManager {
    ...
    private final class TargetQueue {
        ...
        QueueSnapshot snapshot(){List<JobSnapshot> jobs;synchronized(monitor){jobs=java.util.stream.Stream.concat(pending.stream(),activeJobs.values().stream()).distinct().sorted(Comparator.comparingInt((QueuedCall c)->c.priority).reversed().thenComparing(c->c.createdAt)).map(QueuedCall::snapshot).toList();}QuotaState q=quota(group());return new QueueSnapshot(targetId,config.getProvider(),config.getModel(),group(),Math.max(1,config.getQueue().getConcurrency()),(int)jobs.stream().filter(j->j.status()!=JobStatus.RUNNING).count(),running.get(),jobs.stream().filter(j->j.status()==JobStatus.RETRY_WAIT).count(),succeeded.get(),failed.get(),q.blockedUntil(),q.reason(),lastError,lastStatusCode,lastErrorAt,jobs);}
