...
public class AiAutonomousWorkAdminController {
    private final AiAutonomousWorkQueue queue; private final AiGenerationAdmissionService admission; private final AiTargetExecutionQueueManager executionQueues; private final AiCommunityMediaWorker mediaWorker; private final AiSettingsRepository settingsRepository; private final ReplayAnalysisBackfillService replayAnalysis;
    @GetMapping public Overview overview(){return new Overview(autonomousExecutionEnabled(),queue.snapshot(),admission.usageSnapshot(),executionQueues.snapshots(),mediaWorker.snapshot(),replayAnalysis.snapshot());}
    ...
    public record Overview(boolean autonomousExecutionEnabled,AiAutonomousWorkQueue.QueueSnapshot queue,AiGenerationAdmissionService.UsageSnapshot generationUsage,java.util.List<AiTargetExecutionQueueManager.QueueSnapshot> executionQueues,AiCommunityMediaWorker.MediaQueueSnapshot mediaQueue,ReplayAnalysisBackfillService.QueueSnapshot replayAnalysisQueue){}
}
