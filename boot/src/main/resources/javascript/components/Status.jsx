...
const StatusIcon = (status1, status2, maintenance) => {
    if (status1 && status2 && (!maintenance || maintenance === null || maintenance.length === 0) ) {
        return <CheckCircleIcon size="sm" color="green"/>
    }
    const color = status1 || status2 ? "orange" : "red";
    ...
    return <>
        {status === null ?
            ...
            <Popover>
                <PopoverTrigger>
                    <Link>
                        <StatusIcon status1={status.gameServerDatabase}
                                    status2={status.gameServerAddressDirectory}
                                    maintenance={[].concat(status.maintenance.pc, status.maintenance.microsoft, status.maintenance.sony)}/>
                    </Link>
                </PopoverTrigger>
                <PopoverContent>
                    <PopoverArrow/>
                    ...
                    <PopoverFooter>
                        Last check: {status !== null ? status.lastCheck : '-'}
                    </PopoverFooter>
                </PopoverContent>
            </Popover>
        }
