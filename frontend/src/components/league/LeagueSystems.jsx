      ...
      <Accordion allowMultiple defaultIndex={[0]}>
        {leagueSystems.map((leagueSystem) => (
          <AccordionItem key={leagueSystem.id}>
            ...
            <AccordionPanel>
              {selectedSeason(leagueSystem) && <><Heading size="sm" mb={2}>{selectedSeason(leagueSystem).name || `Season ${selectedSeason(leagueSystem).number}`}</Heading><VStack align="stretch" spacing={2} mb={4}>{(selectedSeason(leagueSystem).phases || []).map((phase) => <Box key={phase.id || 'legacy'} borderLeftWidth="3px" pl={3}><Text fontWeight="bold">{phase.name}</Text><Text color="gray.500" fontSize="sm">{(phase.stages || []).map((stage) => stage.name || stage.id).join(' · ') || 'No stages'}</Text></Box>)}{(selectedSeason(leagueSystem).phases || []).length === 0 && <Text color="gray.500">No phases</Text>}</VStack></>}
