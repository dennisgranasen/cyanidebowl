  ...
  return <VStack align="stretch"><Navigation currentPage="account" />
    <Stack spacing={6} maxW="3xl">
      ...
      <Box borderWidth="1px" borderRadius="md" p={5}>
        ...
        <Stack>{candidates.map(candidate=><Box key={candidate.coachId} w="full" minW={0} borderWidth="1px" borderRadius="md" p={3}><Checkbox w="full" alignItems="flex-start" whiteSpace="normal" isChecked={selected.includes(candidate.coachId)} onChange={()=>toggle(candidate.coachId)}><Text as="span" fontWeight="bold" whiteSpace="normal" overflowWrap="anywhere">{candidate.coachName}</Text></Checkbox><Text fontSize="sm" color="gray.500" whiteSpace="normal" overflowWrap="anywhere">{intl.formatMessage({ id: 'account.teamCount' }, { count: candidate.teamCount })}{candidate.teamNames?.length?` · ${candidate.teamNames.join(', ')}`:''}</Text></Box>)}</Stack>
