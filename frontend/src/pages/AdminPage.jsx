...
function AdminPage() {
  ...
  const selectSystem = (item) => { setSelectedSystemId(item.id); setSystem(item); setSelectedSeasonId(null); setSelectedStageId(null); setSelectedSourceId(null); setSeason(emptySeason); setStage(emptyStage); setSource(emptySource); setStages([]); setSources([]); WarpScoresApiService.seasons(item.id, ...auth).then(setSeasons).catch(fail); };
  ...
  const selectStage = (item) => { setSelectedStageId(item.id); setStage(item); setSelectedSourceId(null); setSource(emptySource); WarpScoresApiService.stageSources(item.id, ...auth).then(setSources).catch(fail); };
