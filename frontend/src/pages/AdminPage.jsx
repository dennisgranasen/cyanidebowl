...
function AdminPage() {
  ...
  const selectSeason = (item) => { setSelectedSeasonId(item.id); setSeason(item); setSelectedStageId(null); setSelectedSourceId(null); setStage(emptyStage); setSource(emptySource); setSources([]); clearSourceMatches(); WarpScoresApiService.stages(item.id, ...auth).then(setStages).catch(fail); };
