...
function AdminPage() {
  ...
  const selectSeason = (item) => { setSelectedSeasonId(item.id); setSeason(item); setSelectedPhaseId(null); setSelectedStageId(null); setSelectedSourceId(null); setPhase(emptyPhase); setStage(emptyStage); setSource(emptySource); setStages([]); setSources([]); clearSourceMatches(); WarpScoresApiService.phases(item.id, ...auth).then(setPhases).catch(fail); WarpScoresApiService.registeredSources(item.id, ...auth).then(setRegisteredSources).catch(fail); };
  ...
  const registerCandidate = (competition) => {
    ...
    }, ...auth).then((item) => setRegisteredSources([...registeredSources, item])).catch(fail);
  };
  ...
  return (
    <VStack align="stretch" spacing={5}>
      ...
      {selectedSeasonId && <Box><Heading size="sm" mb={2}>Registered and watched sources</Heading><VStack align="stretch">{registeredSources.map((item) => <Box key={item.id} borderWidth="1px" borderRadius="md" p={3}><HStack justify="space-between"><Box><Text fontWeight="bold">{item.sourceEntityId?.key}</Text><Text color="gray.500" fontSize="sm">{item.game} / {item.platform} · {item.collectionEnabled ? 'Watched' : 'Not watched'}</Text></Box>{selectedStageId && <Button size="sm" onClick={() => attachSource(item)}>Use in selected stage</Button>}</HStack></Box>)}</VStack></Box>}
